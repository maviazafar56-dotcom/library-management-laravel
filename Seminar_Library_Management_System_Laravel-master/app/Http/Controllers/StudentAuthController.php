...
class StudentAuthController extends Controller
{
    ...
    public function sign_up_process(Request $req)
    {
        ...
        $email_check=Student::where('Email',$req->email)->count();

        $username_check=Student::where('Username',$req->username)->count();

        $student_id_check=Student::where('Student_ID',$req->student_id)->count();
    ...
    public function confirm_email(Request $req)
    {
        ...
        {
            $notification = array(
                'message' => 'Session expired or student not found. Please sign up again.',
                'alert-type' => 'error'
            );
            return Redirect::to('student/signup')->with($notification);
        }
        ...
        if($req->code == $student->Confirmation_Code)
        {
            ...
            $update_status=Student::where('id',$id)->update($data);
    ...
    public function forget_password_process(Request $req)
    {
        ...
        $email=Student::where('Email',$req->email)->count();
    ...
    public function recover_password_process(Request $req)
    {
        ...
        $student=Student::where('Email',$email)->first();
            ...
            if($req->new_password == $req->confirm_password)
            {
                ...
                $data['Password']=$pass;
                $update_password=Student::where('Email',$email)->update($data);
    ...
    public function sign_in_process(Request $req)
    {
        ...
        $username = Student::where('Username',$req->email)->count();
        ...
        if($email > 0 || $username > 0)
        {
                ...
                if($username > 0)
                {
                    ...
                    $student = Student::where('Username',$req->email)->first();
                ...
                if(Hash::check($req->password,$student->Password))
                {
                    ...
                    if($student->Verify == "Pending" || $student->Verify == "Panding")
                    {
                    ...
                    else if($student->Verify == "No")
                    {
                        ...
                        $update_code=Student::where('id',$id)->update($data);
    ...
    public function change_password_process(Request $req)
    {
        ...
        $student_account=Student::find($student);
        ...
        if(Hash::check($req->old_password,$student_account->Password))
        {
            ...
            if($req->new_password==$req->confirm_password)
            {
                ...
                $update_password=Student::where('id',$student)->update($data);
    ...
    public function edit_info_process(Request $req,$id)
    {
        ...
        $check_username=Student::where('Username',$username)
        ->where('id','<>',$id)
        ->count();
        ...
        $update_info=Student::where('id',$id)
        ->update($data);
